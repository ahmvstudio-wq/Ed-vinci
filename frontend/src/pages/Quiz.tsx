...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    optionA: { label: 'I need clarity before starting.', sub: 'I want the full picture first.', icon: '🗺️' },
    optionB: { label: 'I learn best by jumping in.', sub: 'I figure it out as I go.', icon: '🏄' },
  },
  {
    id: 'energy-pick',
    type: 'energy-pick',
  ...
  },
  {
    ...
    optionA: { label: 'I want results within weeks.', sub: 'Fast wins motivate me.', icon: '⚡' },
    optionB: { label: "I'm building something long-term.", sub: 'I think in months and years.', icon: '🌱' },
  },
  {
    id: 'final-insight',
    type: 'final-insight',
...
// ─── INTERSTITIAL SCREEN (Four Masters Paradox) ───────────────────────────────

function InterstitialScreen({ onContinue }: { onContinue: () => void }) {
  ...
  return (
    <div className="quiz-interstitial-root">
      <div className="quiz-interstitial-orbs">
        <motion.div
          className="quiz-interstitial-orb quiz-interstitial-orb-1"
          animate={{ scale: [1, 1.2, 1], x: [0, 20, -10, 0], y: [0, -15, 10, 0] }}
          transition={{ duration: 10, repeat: Infinity, ease: 'easeInOut' }}
        />
        <motion.div
          className="quiz-interstitial-orb quiz-interstitial-orb-2"
          animate={{ scale: [1.05, 1, 1.1], x: [0, -18, 12, 0], y: [0, 18, -12, 0] }}
          transition={{ duration: 14, repeat: Infinity, ease: 'easeInOut' }}
        />
      </div>

      <div className="quiz-interstitial-content">
        <AnimatePresence>
          {phase >= 1 && (
            <motion.div
              className="quiz-interstitial-avatar"
        ...
        <div className="quiz-interstitial-lines">
          <AnimatePresence>
            {phase >= 2 && (
              <motion.p
                className="quiz-interstitial-line quiz-interstitial-line-lead"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
              >
                Most people who feel ready for AI have built a very clear picture…
              </motion.p>
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {phase >= 3 && (
              <motion.p
                className="quiz-interstitial-line quiz-interstitial-line-punch"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
              >
          ...
          <AnimatePresence>
            {phase >= 4 && (
              <motion.p
                className="quiz-interstitial-line quiz-interstitial-line-sub"
                initial={{ opacity: 0, y: 14 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
              >
                Let's find out where you actually are. Five questions. Three minutes.
              </motion.p>
            )}
          </AnimatePresence>
        </div>
        ...
        <AnimatePresence>
          {phase >= 4 && (
            <motion.button
              className="quiz-interstitial-cta"
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              whileHover={{ scale: 1.04, boxShadow: '0 8px 40px rgba(79,142,247,0.28)' }}
              whileTap={{ scale: 0.97 }}
              transition={{ duration: 0.6, delay: 0.15, ease: [0.22, 1, 0.36, 1] }}
              onClick={onContinue}
            >
              <span>Show me where I'm actually at</span>
    ...
    <div className="quiz-dots">
      {inputSteps.map((_, i) => (
        <motion.div
          ...
          animate={{ width: i === inputCurrent - 1 ? 24 : 8, opacity: i < inputCurrent ? 1 : 0.25 }}
      ...
      <div className="quiz-binary-row">
        {[step.optionA!, step.optionB!].map((opt, i) => {
          ...
          return (
            <motion.button
              key={i}
              className={`quiz-binary-card ${isSelected ? 'quiz-binary-selected' : ''} ${isDimmed ? 'quiz-binary-dimmed' : ''}`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
...
function EnergyPickStep({ step, onAnswer }: { step: Step; onAnswer: (v: string) => void }) {
  ...
  const colors = ['quiz-word-blue', 'quiz-word-pink', 'quiz-word-amber', 'quiz-word-teal', 'quiz-word-violet', 'quiz-word-rose', 'quiz-word-indigo', 'quiz-word-emerald'];
...
function InsightStep({ step, onDone }: { step: Step; onDone: () => void }) {
...
export function Quiz() {
  const [quizPhase, setQuizPhase] = useState<'interstitial' | 'quiz'>('interstitial');
  ...
  const submitQuiz = (finalAnswers: Record<string, string>) => {
    ...
    try {
      ...
      if (onboardingData) {
        ...
        if (parsed.answers) onboardingAnswers = parsed.answers;
      }
    } catch (e) {
    ...
    navigate('/processing', {
      state: { answers: { ...finalAnswers, ...onboardingAnswers, sessionId } },
    });
  };
  ...
  // ── Four Masters Paradox interstitial gate ──────────────────────────────────
  if (quizPhase === 'interstitial') {
    return (
      <div className="quiz-root">
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <InterstitialScreen onContinue={() => setQuizPhase('quiz')} />
          </motion.div>
        </AnimatePresence>
      </div>
    );
    ...
    <div className="quiz-root">
      ...
      <main className={`quiz-main ${isInsight ? 'quiz-main-insight' : ''}`}>
        <AnimatePresence mode="wait">
          ...
          >
            {currentStep.type === 'build-cards' && <BuildCardsStep step={currentStep} onAnswer={handleAnswer} />}
            {currentStep.type === 'multi-choice' && <MultiChoiceStep step={currentStep} onAnswer={handleAnswer} />}
            {currentStep.type === 'binary' && <BinaryStep step={currentStep} onAnswer={handleAnswer} />}
            {currentStep.type === 'energy-pick' && <EnergyPickStep step={currentStep} onAnswer={handleAnswer} />}
